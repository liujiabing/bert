...
#CUDA_VISIBLE_DEVICES=0,1 python run_classifier.py --task_name=session --do_train=true --do_eval=true --data_dir=$DATA_DIR --vocab_file=$BERT_BASE_DIR/vocab.txt --bert_config_file=$BERT_BASE_DIR/bert_config.json --init_checkpoint=$BERT_BASE_DIR/bert_model.ckpt --max_seq_length=150 --train_batch_size=32 --learning_rate=2e-5 --num_train_epochs=2.0 --output_dir=$DIR/bert-session-cls/session_output
